...
# Fonction pour afficher les métriques d'un conteneur individuellement
show_container_individual() {
    local container_name=$1
    local display_name=$2
    local color=$3
    
    echo -e "${color}📊 $display_name - Métriques Individuelles${NC}"
    echo "====================================="
...
main() {
    clear
    echo -e "${WHITE}🚀 DevOps Project - Monitoring Final (Individuel)${NC}"
    echo -e "${WHITE}===============================================${NC}"
    echo ""
    
    # Afficher chaque conteneur individuellement
    show_container_individual "idurar-portainer" "Portainer" "${GREEN}"
    show_container_individual "idurar-mongodb" "MongoDB" "${YELLOW}"
    show_container_individual "idurar-backend" "Backend" "${BLUE}"
    show_container_individual "idurar-frontend" "Frontend" "${PURPLE}"
    ...
    echo ""
    echo -e "${YELLOW}💡 Pour une surveillance continue individuelle, utilisez:${NC}"
    echo -e "${WHITE}   ./devops-individual-final-dashboard.sh --continuous${NC}"
    echo ""
    echo -e "${GREEN}✅ Monitoring individuel actif - Chaque conteneur affiché séparément${NC}"
}
